...
<header class="header fixed top-0 left-0 right-0 z-50 p-4 flex justify-between items-center">
    <div class="flex items-center space-x-4">
        ...
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Formulaire Client</h1>
    </div>
    <div class="flex items-center space-x-4">
        <div class="relative">
...
<div class="fixed top-0 left-0 h-full w-64 sidebar z-40">
    <div class="p-6">
        ...
        <nav class="space-y-2">
            ...
            <a href="ClientServlet" class="sidebar-item flex items-center space-x-3 p-3 bg-gray-100 dark:bg-gray-700">
                <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<main class="md:ml-64 pt-20 p-6 min-h-screen">
    <div class="card p-6 max-w-lg mx-auto">
        <h2 class="text-3xl font-semibold mb-6 text-center text-gray-900 dark:text-white">${editClient != null ? 'Modifier Client' : 'Ajouter Client'}</h2>
        <c:if test="${not empty error}">
            <p class="text-red-500 text-center mb-4">${error}</p>
        </c:if>
        <form method="post" action="ClientServlet" id="clientForm">
            <input type="hidden" name="clientId" value="${editClient != null ? editClient.id : ''}" />
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">NCIN</label>
                <input type="text" name="ncin" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none" value="${editClient != null ? editClient.ncin : ''}" required />
                <p class="text-red-500 text-xs mt-1 hidden error-message">NCIN doit être un numéro.</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Nom</label>
                <input type="text" name="nom" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none" value="${editClient != null ? editClient.nom : ''}" required />
                <p class="text-red-500 text-xs mt-1 hidden error-message">Nom est requis.</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Prénom</label>
                <input type="text" name="prenom" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none" value="${editClient != null ? editClient.prenom : ''}" required />
                <p class="text-red-500 text-xs mt-1 hidden error-message">Prénom est requis.</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Num Tel</label>
                <input type="text" name="numTel" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none" value="${editClient != null ? editClient.numTel : ''}" required />
                <p class="text-red-500 text-xs mt-1 hidden error-message">Num Tel doit être un numéro.</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Email</label>
                <input type="email" name="email" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none" value="${editClient != null ? editClient.email : ''}" required />
                <p class="text-red-500 text-xs mt-1 hidden error-message">Format d'email invalide.</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Adresse</label>
                <input type="text" name="adresse" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none" value="${editClient != null ? editClient.adresse : ''}" required />
                <p class="text-red-500 text-xs mt-1 hidden error-message">Adresse est requise.</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-700 dark:text-gray-300">Num Permis</label>
                <input type="text" name="numPermis" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none" value="${editClient != null ? editClient.numPermis : ''}" required />
                <p class="text-red-500 text-xs mt-1 hidden error-message">Num Permis doit être un numéro.</p>
            </div>
            <div class="flex space-x-4">
                <button type="submit" class="btn-primary px-6 py-3 rounded-lg">Soumettre</button>
                <a href="ClientServlet" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-3 rounded-lg transition">Annuler</a>
            </div>
        </form>
    </div>
</main>
...
<script>
    ...
    form.addEventListener('input', (e) => {
        ...
        if (target.name === 'ncin' || target.name === 'numTel' || target.name === 'numPermis') {
