...
<header class="header fixed top-0 left-0 right-0 z-50 p-4 flex justify-between items-center">
    ...
    <div class="flex items-center space-x-4">
        <div class="relative">
            <input type="text" id="search" placeholder="Rechercher par nom..." class="pl-10 pr-4 py-2 rounded-lg bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500">
...
<main class="md:ml-64 pt-20 p-6 min-h-screen">
    <div class="card p-6">
        <h2 class="text-3xl font-semibold mb-6 text-center text-gray-900 dark:text-white">Clients</h2>
        <div class="mb-4">
            <a href="ClientServlet?action=add" class="btn-primary px-4 py-2 rounded-lg">Ajouter un Client</a>
        </div>
        <c:choose>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr>
                            ...
                            <th class="py-3 px-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">NCIN</th>
                            <th class="py-3 px-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Nom</th>
                            <th class="py-3 px-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Prénom</th>
                            <th class="py-3 px-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Num Tel</th>
                            <th class="py-3 px-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Email</th>
                            <th class="py-3 px-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Adresse</th>
                            <th class="py-3 px-4 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Num Permis</th>
                        ...
                        <c:forEach var="client" items="${clients}">
                            <tr class="client-row">
                                <td class="py-3 px-4 text-gray-900 dark:text-white">${client.id}</td>
                                <td class="py-3 px-4 text-gray-900 dark:text-white">${client.ncin}</td>
                                <td class="py-3 px-4 text-gray-900 dark:text-white">${client.nom}</td>
                                <td class="py-3 px-4 text-gray-900 dark:text-white">${client.prenom}</td>
                                <td class="py-3 px-4 text-gray-900 dark:text-white">${client.numTel}</td>
                                <td class="py-3 px-4 text-gray-900 dark:text-white">${client.email}</td>
                                <td class="py-3 px-4 text-gray-900 dark:text-white">${client.adresse}</td>
                                <td class="py-3 px-4 text-gray-900 dark:text-white">${client.numPermis}</td>
                                <td class="py-3 px-4 flex space-x-2">
                                    <div class="relative tooltip-parent">
                                        <a href="ClientServlet?action=edit&id=${client.id}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded-lg transition">Modifier</a>
                                        <span class="tooltip absolute bg-gray-800 text-white text-xs rounded p-2 -mt-8">Modifier Client</span>
                                    </div>
                                    <div class="relative tooltip-parent">
                                        <button onclick="showModal(${client.id})" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-lg transition">Supprimer</button>
                                        <span class="tooltip absolute bg-gray-800 text-white text-xs rounded p-2 -mt-8">Supprimer Client</span>
                                    </div>
                                </td>
                            </tr>
                        </c:forEach>
            ...
            <c:otherwise>
                <p class="text-center text-gray-600 dark:text-gray-400">Aucun client disponible.</p>
            </c:otherwise>
        </c:choose>
    </div>
</main>
...
<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg modal">
        <h3 class="text-lg font-semibold mb-4 text-gray-900 dark:text-white">Confirmer la Suppression</h3>
        <p>Êtes-vous sûr de vouloir supprimer ce client ?</p>
        <div class="flex space-x-4 mt-6">
            <a id="confirmDelete" href="#" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg transition">Oui, Supprimer</a>
            <button onclick="hideModal()" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg transition">Annuler</button>
        </div>
    </div>
</div>
...
<script>
    ...
    searchInput.addEventListener('input', () => {
        ...
        rows.forEach(row => {
            ...
            const prenom = row.querySelector('td:nth-child(4)').textContent.toLowerCase();
    ...
    function showModal(id) {
        ...
        confirmDelete.href = `ClientServlet?action=delete&id=${id}`;
        modal.classList.remove('hidden');
        gsap.fromTo(modal.querySelector('.modal'), { opacity: 0, scale: 0.9 }, { opacity: 1, scale: 1, duration: 0.3 });
    }

    function hideModal() {
        const modal = document.getElementById('deleteModal');
        gsap.to(modal.querySelector('.modal'), { opacity: 0, scale: 0.9, duration: 0.3, onComplete: () => modal.classList.add('hidden') });
    }
</script>
</body>
</html>