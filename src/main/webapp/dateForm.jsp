...
<head>
    ...
    <title>${editDate != null ? 'Modifier Location' : 'Ajouter Location'}</title>
...
<header class="header fixed top-0 left-0 right-0 z-50 p-4 flex justify-between items-center">
    <div class="flex items-center space-x-4">
        ...
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Formulaire de Location</h1>
    </div>
    <div class="flex items-center space-x-4">
        <div class="relative">
...
<main class="md:ml-64 pt-20 p-6 min-h-screen">
    <div class="card p-6 max-w-lg mx-auto">
        <h2 class="text-2xl font-semibold mb-6 text-gray-900 dark:text-white">${editDate != null ? 'Modifier Location' : 'Ajouter Location'}</h2>
        <c:if test="${not empty error}">
            <p class="text-red-500 mb-4">${error}</p>
        </c:if>
        <form method="post" action="DateServlet" id="rentalForm">
            <input type="hidden" name="id" value="${editDate != null ? editDate.id : ''}" />
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-900 dark:text-white">Client</label>
                <select name="clientId" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none" required>
                    <option value="">Sélectionner un client</option>
                    <c:forEach var="client" items="${clients}">
                        <option value="${client.id}" ${editDate != null && editDate.client.id == client.id ? 'selected' : ''}>${client.nom} ${client.prenom}</option>
                    </c:forEach>
                </select>
                <p class="text-red-500 text-xs mt-1 hidden error-message">Le client est requis.</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-900 dark:text-white">Voiture</label>
                <select name="voitureId" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none" required>
                    <option value="">Sélectionner une voiture</option>
                    <c:forEach var="car" items="${voitures}">
                        <option value="${car.id}" ${editDate != null && editDate.voiture.id == car.id ? 'selected' : ''}>${car.marque} ${car.modele}</option>
                    </c:forEach>
                </select>
                <p class="text-red-500 text-xs mt-1 hidden error-message">La voiture est requise.</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-900 dark:text-white">Date de début</label>
                <input type="date" name="dateDebut" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none" value="${editDate != null ? editDate.dateDebut : ''}" required />
                <p class="text-red-500 text-xs mt-1 hidden error-message">La date de début est requise.</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-900 dark:text-white">Date de fin</label>
                <input type="date" name="dateFin" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none" value="${editDate != null ? editDate.dateFin : ''}" />
                <p class="text-red-500 text-xs mt-1 hidden error-message">La date de fin doit être postérieure à la date de début.</p>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-medium mb-2 text-gray-900 dark:text-white">Satisfaction (1-5)</label>
                <select name="satisfaction" class="w-full p-3 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 focus:outline-none">
                    <option value="">Non évalué</option>
                    <c:forEach var="i" begin="1" end="5">
                        <option value="${i}" ${editDate != null && editDate.satisfaction == i ? 'selected' : ''}>${i}</option>
                    </c:forEach>
                </select>
                <p class="text-red-500 text-xs mt-1 hidden error-message">La satisfaction doit être entre 1 et 5.</p>
            </div>
            <div class="flex space-x-4">
                <button type="submit" class="btn-primary px-6 py-3 rounded-lg">Enregistrer</button>
                <a href="DateServlet" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-3 rounded-lg transition">Annuler</a>
            </div>
        </form>
    </div>
</main>
...
<script>
    ...
    form.addEventListener('input', (e) => {
        ...
        } else if (target.name === 'dateFin' && target.value) {
            const dateDebut = document.querySelector('input[name="dateDebut"]').value;
            if (dateDebut && target.value < dateDebut) {
